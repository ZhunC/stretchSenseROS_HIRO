...
class ROSHandler:
    ...
    def plot_finger_states(self):
        ...
        if not all(len(lst) == num_states for lst in [self.index_states, self.middle_states, self.ring_states, self.pinky_states, self.iter]):
            raise ValueError("All state lists must have the same length.")

        # Plot each finger's states
        plt.figure(figsize=(10, 6))
        plt.plot(self.iter, self.thumb_yaw_states, label="Thumb yaw", marker='+')
        plt.plot(self.iter, self.thumb_pitch_states, label="Thumb Pitch", marker='o')
