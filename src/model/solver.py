...
class LeastSquaresSolver:
    ...
    def solve(self,
        ...
        for index, targets in enumerate(target_data_transposed):
            ...
            # modified version, using raw input
            regr.fit(input_data[:, 1:], targets)

            print(f'solved coefficents have length {len(regr.coef_)}')
    ...
    def _clean_input(self,
        ...
        num_sensors = len(active_sensors[0]) - 1 # accounting for the patching of +1 in sensor reaction matrix in stretchsense_peripheral.py

        # Pad with zeroes to ensure output always has 3 columns (modified)
        # Modified such that the output always has equal amount of columns as number of sensors
        return np.pad(cleaned_input,
                      ((0,0),(0,num_sensors-cols)),
                      'constant')