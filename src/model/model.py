...
class Model:
    ...
    def apply_transformation(self,
        ...
        # cleaned_input = np.ones((len(input_data), 4))  # for 10 sensor glove, cleaned_input is 11-by-4 

        num_sensors = len(self._peripheral.ACTIVE_SENSORS[0])
        cleaned_input = np.ones((len(input_data), num_sensors)) # modified from original code, now size is\ 11-by-11 for 10 sensor gloves
        ...
        for i in range(len(self._theta)):
            ...
            # modified version, using raw input data
            # moving the interception coefficient 1 from start to the end of the array to keep in accordance with .csv weights
            raw_input = np.concatenate((input_data[1:], input_data[:1])) 
            print(input_data)
            print(raw_input)
            result[i] = np.dot(self._theta[i], raw_input)
